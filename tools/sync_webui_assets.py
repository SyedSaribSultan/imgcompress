...
def module(name: str) -> str:
    ...
    No path rewriting is needed and that is not an accident: every import
    between these files is `./sibling.js`, and the copies land flat in one
    directory, so the graph resolves identically under `/webui/`.
    ...
    """
    return BANNER.format(name=name) + _read_exact(WEB / name)
